...
MASTERDATA_PATH   = "masterdata.json"
...
NOTICE_ZH = """
    <p>1. 本网站为リステップ用户数据保存工具，保存用户几乎所有数据（除了登录奖励进度不保存），在 7月31日12:00 服务器关闭之前均可正常使用。本工具仅用于学习与技术研究目的，不用于任何商业用途。使用过程中如出现数据异常、账号异常或其他任何问题，开发者概不负责。</p>
    ...
    <p>使用方法：输入用户id和继承码以后，先点击左侧按钮，该按钮用于保存除"历史活动排名"之外的全部用户数据，执行速度较快（30s以内）；右侧按钮用于保存全部历史活动排名数据，可以保存也可以不保存，由于数据量较大，执行时间可能较长（约5-10分钟），请耐心等待。</p>
    <p>数据备份完成后，需要3-5分钟才能同步到私服，待3-5分钟以后再在私服上操作，直接使用网页显示的继承码继承即可</p>
    <p>2. 本项目代码已完全开源，GitHub 地址：https://github.com/qdefgdfdg/step-user-data-backup.git。欢迎学习与二次开发，但请遵守开源协议。</p>
    <p>3. 若使用过程中存在问题，可以在github的issue留言；uuid是当继承码无法显示时，救回账户的唯一途径，因此请务必截图！</p>
"""

NOTICE_JA = """
    <p>1. 本サイトはリステップのユーザーデータ保存ツールです。ユーザーのほぼ全てのデータを保存します（ただしログインボーナス進捗は保存対象外です）。7月31日12:00のサービス終了まで利用可能です。本サイトは学習・技術研究目的のみに使用され、商業目的には使用しません。利用中にデータ異常・アカウント異常などが発生した場合、開発者は一切の責任を負いません。</p>
    <p class="red">⚠️ サードパーティツールの使用はゲームの利用規約に違反する可能性があり、アカウント制限・停止のリスクがあります。すべて自己責任でご利用ください。</p>
    <p class="red">⚠️ 左側のボタンをクリックすると、新しい引継ぎパスワードが表示されます。必ずすぐにスクリーンショットを撮って保存してください！画面を更新しないでください！</p>
    <p class="red">⚠️ 本サイトはAndroidスマートフォン専用です！iOS（Apple）端末で使用すると、有償ジュエルデータの異常・消失が発生する可能性がありますので、十分ご注意ください！</p>
    <p>使用方法：ユーザーIDと引継ぎパスワードを入力後、まず左側のボタンをクリックしてください。このボタンは「過去イベントランキング」を除くすべてのユーザーデータを保存します（処理は約30秒以内）。右側のボタンはすべての過去イベントランキングデータを保存します。データ量が多いため、処理には時間がかかる場合があります（約5〜10分）。完了するまでそのままお待ちください。</p>
    <p>データのバックアップ完了後、私設サーバーへの同期には3〜5分程度かかります。そのため、3〜5分後に私設サーバー上で操作を行い、表示された引継ぎコードをそのまま使用してアカウントを引き継くことが可能です。</p>
    <p>2. 本プロジェクトのコードは完全にオープンソースです。GitHub：https://github.com/qdefgdfdg/step-user-data-backup.git。学習・二次開発は歓迎しますが、オープンソースライセンスを必ず遵守してください。</p>
    <p>3. 利用中に問題が発生した場合は、GitHubのIssueにてご報告ください。UUIDは引継ぎパスワードが表示されない場合にアカウントを復旧する唯一の手段ですので、必ず保存してください。</p>
"""

# ── HTML テンプレート ─────────────────────────────────────
def make_html(lang="zh"):
    lang_link = '<a href="/ja">🇯🇵 日本語版はこちら</a>' if lang == "zh" else '<a href="/">🇨🇳 中国語版はこちら</a>'
    title     = "用户数据迁移工具" if lang == "zh" else "ユーザーデータ移行サイト"
    lbl_id    = "用户 ID（player_id）" if lang == "zh" else "ユーザーID（player_id）"
    lbl_code  = "继承码" if lang == "zh" else "引継ぎパスワード"
    btn_mig   = "读取基础数据" if lang == "zh" else "基礎データを読み込む"
    btn_evt   = "读取过往活动排名" if lang == "zh" else "イベントランキング取得"
    btn_dl1   = "⬇ 下载基础数据" if lang == "zh" else "⬇ 基礎データをダウンロード"
    btn_dl2   = "⬇ 下载过往活动排名数据" if lang == "zh" else "⬇ イベントランキングをダウンロード"
    hl_code   = "新引继码" if lang == "zh" else "新しい引継ぎパスワード"
    hl_uuid   = "UUID（请务必截图）" if lang == "zh" else "UUID（必ずスクショを撮ってください）"
    notice_h  = "⚠️ 注意事项" if lang == "zh" else "⚠️ 注意事項"
    notice_c  = NOTICE_ZH if lang == "zh" else NOTICE_JA
    alert1    = "请填写用户ID和继承码" if lang == "zh" else "ユーザーIDと引継ぎパスワードを入力してください"
    alert2    = "请先完成左边的迁移数据步骤" if lang == "zh" else "先に左側の基礎データを読み込んでください"
